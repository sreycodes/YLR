...
def construct_feed_dict(adj_normalized, adj, features, placeholders): #, features_nonzero, num_nodes):
...
def get_bridge_edges(adj):
    ...
    bridges = []
    for at in range(num_vertices):
        if not visited[at]:
            dfs(i, at, -1, bridges, visited, ids, low, adj)
            i += 1
    return bridges


def dfs(i, at, parent, bridges, visited, ids, low, adj):
    visited[at] = True
    low[at] = ids[at] = i
    for to, v in enumerate(adj[at]):
        if v != 0:
            if to is parent:
                continue
            else:
                if not visited[to]:
                    dfs(i + 1, to, at, bridges, visited, ids, low, adj)
...
def mask_test_edges(adj, test_frac=.1, val_frac=.05, prevent_disconnect=True, verbose=False):
    ...
    test_edges = set()
    val_edges = set()   
    bridge_edges = set(nx.bridges(nx.from_scipy_sparse_matrix(adj)))
    ...
    for edge in edge_tuples:
        ...
        if len(test_edges) < num_test:
            ...
            adj_train[node1, node2] = 0
            adj_train[node2, node1] = 0
    ...
    while len(train_edges_false) < len(train_edges):
        ...
        # adj_train[idx_i, idx_j] = -1
        # adj_train[idx_j, idx_i] = -1
