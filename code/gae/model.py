from gae.layers import GraphConvolution, GraphConvolutionSparse, InnerProductDecoder, GravityInspiredDecoder
...
class GCNModelVAE(Model):
    def __init__(self, placeholders, num_features, num_nodes, features_nonzero, 
      hidden1_dim=32, hidden2_dim=16, flatten_output=True, dtype=tf.float32, **kwargs):
        ...
        self.inputs = placeholders['features']
        self.input_dim = num_features
        self.features_nonzero = features_nonzero #placeholders['features_nonzero']
        self.adj = placeholders['adj']
        self.n_samples = num_nodes #placeholders['num_nodes']
        self.dropout = placeholders['dropout']
        self.hidden1_dim = hidden1_dim
        self.hidden2_dim = hidden2_dim
...
# Graph Variational Auto-Encoder model for directed graphs
class DirectedGCNModelVAE(Model):
    def __init__(self, placeholders, num_features, num_nodes, features_nonzero, 
      hidden1_dim=32, hidden2_dim=16, flatten_output=True, dtype=tf.float32, **kwargs):
        super(DirectedGCNModelVAE, self).__init__(**kwargs)
    ...
    def _build(self):
        ...
        # Sampling operation: z = z_mean + (random_noise_factor) * z_stddev
        self.z = self.z_mean + tf.random_normal([self.n_samples, self.hidden2_dim + 1], dtype=self.dtype) * tf.exp(self.z_log_std)

        # Inner-Product Decoder: Z (embeddings) --> A (reconstructed adj.)
        self.reconstructions, self.o1, self.o2, self.m, self.im, self.d = GravityInspiredDecoder(input_dim = self.hidden2_dim + 1,
                                                      act=tf.nn.sigmoid)(self.z)
